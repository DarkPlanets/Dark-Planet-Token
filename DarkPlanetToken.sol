...
interface IMultiSignature {
    function is_apporved(uint) external view returns (uint,string memory, uint, address, bool);
    function get_dao_delegate_amount()external view returns(uint); 
}
...
abstract contract Ownable is Context {
     ...
     */
    function configMultisignature(address sign) public virtual onlyOwner {
        _ms = IMultiSignature(sign);
    }
...
contract DarkPlanetToken is ERC20, IDarkPlanetERC20, Ownable {
    ...
    function whitelist(uint proposalIndex) external override {
        ...
        (m_type,applySymbol, approved, operator, arg) = _ms.is_apporved(proposalIndex);
        
        require(m_type == 1,"Proposal type error .");
        ...
        require(approved == _ms.get_dao_delegate_amount(), "The proposal will be executed after all delegates have passed .");
